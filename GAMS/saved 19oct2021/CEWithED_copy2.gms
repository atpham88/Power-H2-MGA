...
*       limitstorageresupH2tech(storagetech,h)
        limitStoH2TechGen(hydrogentech,h)
                limitStoH2TechCharge(hydrogentech,h)
                stoH2TechSOCLimit(hydrogentech,h)
        limitstorageresupH2tech(batterytech,h)
        limitStoBatTechGen(batterytech,h)
                limitStoBatTechCharge(batterytech,h)
                stoBatTechSOCLimit(batterytech,h)
        limitstorageresupBattech(batterytech,h)
*CO2 emissions cap
...
limitStoBatTechGen(batterytech,h) .. vGentech(batterytech,h) =l= pStoinenergymarket*vPowBuiltBatSto(batterytech);
limitStoBatTechCharge(batterytech,h) .. vChargeBattech(batterytech,h) =l= vPowBuiltBatSto(batterytech)*pChargeDischargeCapRatio;
stoBatTechSOCLimit(batterytech,h) .. vStateofchargeBattech(batterytech,h) =l= vEneBuiltBatSto(batterytech);

limitStoH2TechGen(hydrogentech,h) .. vGentech(hydrogentech,h) =l= pStoinenergymarket*vPowBuiltH2Sto(hydrogentech);
limitStoH2TechCharge(hydrogentech,h) .. vChargeH2tech(hydrogentech,h) =l= vPowBuiltH2Sto(hydrogentech)*pChargeDischargeCapRatio;
stoH2TechSOCLimit(hydrogentech,h) .. vStateofchargeH2tech(hydrogentech,h) =l= vEneBuiltH2Sto(hydrogentech);
...
limitstorageresupBattech(batterytech,h) .. vReguptech(batterytech,h) + vFlextech(batterytech,h) + vConttech(batterytech,h) =l= (vPowBuiltBatSto(batterytech) - vGentech(batterytech,h)) + vChargeBattech(batterytech,h);
limitstorageresupH2tech(hydrogentech,h) .. vReguptech(hydrogentech,h) + vFlextech(hydrogentech,h) + vConttech(hydrogentech,h) =l= (vPowBuiltH2Sto(hydrogentech) - vGentech(hydrogentech,h)) + vChargeH2tech(hydrogentech,h);
