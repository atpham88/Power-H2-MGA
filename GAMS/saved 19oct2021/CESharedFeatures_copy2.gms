...
*Candidate plants for construction
         ...
         hydrogentech(tech)               storage plant types for new construction
         batterytech(tech)
...
$gdxin %gdxincname%
$load ststorageegu, ltstorageegu, tech, dacstech, CCStech, thermaltech, nucleartech, CCtech, renewtech, solartech, hydrogentech, batterytech, windtech, storagetech, ststoragetech, ltstoragetech, peakh
...
$load pFom, pOcc, pPowOcc, pPowH2Occ, pPowBatOcc, pEneOcc, pEneH2Occ, pEneBatOcc, pRampratetech, pCO2emratetech
$load pEfficiencytech ,pEfficiencyH2tech, pEfficiencyBattech
...
*                 vEneBuiltSto(storagetech)                      built energy capacity for storage
                 vStateofchargeH2tech(hydrogentech,h)            "energy stored in storage unit at end of hour h (GWh)"
         vChargeH2tech(hydrogentech,h)                           "charged energy by storage unit in hour h (GWh)"
                 vPowBuiltH2Sto(hydrogentech)                      built power capacity for storage
                 vEneBuiltH2Sto(hydrogentech)                      built energy capacity for storage
                 vStateofchargeBattech(batterytech,h)            "energy stored in storage unit at end of hour h (GWh)"
         vChargeBattech(batterytech,h)                           "charged energy by storage unit in hour h (GWh)"
                 vPowBuiltBatSto(batterytech)                      built power capacity for storage
                 vEneBuiltBatSto(batterytech)                      built energy capacity for storage
                   ;
...
*                 maxESTSto(ststoragetech)
                 maxELTH2Sto(hydrogentech)
                 maxESTBatSto(batterytech)
*Carbon emissions
...
investmentcost..         vFixedcostannual =e= sum(nonstoragetech,vN(nonstoragetech)*pCapactech(nonstoragetech)*(pFom(nonstoragetech)+pOcc(nonstoragetech)*pCrf(nonstoragetech)))
                                                                                                + sum(hydrogentech,vPowBuiltH2Sto(hydrogentech)*pPowH2Occ(hydrogentech)*pCrf(hydrogentech)+vEneBuiltH2Sto(hydrogentech)*pEneH2Occ(hydrogentech)*pCrf(hydrogentech))
                                                                                                + sum(batterytech,vPowBuiltBatSto(batterytech)*0(batterytech)*pCrf(batterytech)+vEneBuiltBatSto(batterytech)*0(batterytech)*pCrf(batterytech));;
...
meetdemand(h)..          sum(tech,vGentech(tech,h))+sum(egu,vGen(egu,h)) =g= (pDemand(h)
                                                        ...
                                                        + sum(hydrogentech,vChargeH2tech(hydrogentech,h)))
                                                        + sum(batterytech,vChargeBattech(batterytech,h)));
...
meetreservemargin..       sum(nucleartech,pCapactech(nucleartech)*vN(nucleartech))
                           ...
                           + sum(hydrogentech,vPowBuiltH2Sto(hydrogentech))
                           + sum(batterytech,vPowBuiltBatSto(batterytech))
...
setVNH2Sto(hydrogentech) .. vN(hydrogentech) =e= vPowBuiltH2Sto(hydrogentech)/pCapactech(hydrogentech);
setVNBatSto(batterytech) .. vN(batterytech) =e= vPowBuiltBatSto(batterytech)/pCapactech(batterytech);
maxELTH2Sto(hydrogentech) .. vEneBuiltH2Sto(hydrogentech) =l= pPMaxH2Sto;
maxESTBatSto(batterytech) .. vEneBuiltBatSto(batterytech) =e= pPERatio*vPowBuiltBatSto(batterytech);
***************************************************
